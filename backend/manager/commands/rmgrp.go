...
func Fn_Rmgrp(params string) (string, error) {
	...
	if globals.LoginSession.User != "root" {
		utils.ShowMessage("Solo el usuario root puede eliminar grupos.", true)
	...
	return Rmgrp(name, part)
}

// Rmgrp -name=<nombre>
func Rmgrp(name string, part *Structs.Partition) (string, error) {
	...
	if !found {
		utils.ShowMessage("No existe un grupo activo con el nombre '"+name+"'.", true)
		return "", fmt.Errorf("no existe un grupo activo con el nombre '%s'", name)
	}
	...
	utils.WriteJournaling(sb, *part, file, []byte("rmgrp"), part.Name[:], []byte("remove_"+name))

	utils.ShowMessage("Grupo ["+name+"] eliminado exitosamente.", false)
	return "Grupo [" + name + "] eliminado exitosamente.", nil
}
